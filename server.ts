...
`;
    ...
    try {
      if (provider === 'openai') {
        if (!apiKey || !apiKey.trim()) {
          return res.status(400).json({
            success: false,
            error: 'Chave da OpenAI não informada. Por favor, informe sua chave de API (sk-...) nas configurações de IA.'
          });
        }
        rawOutput = await callOpenAI(apiKey.trim(), model, userPrompt, temperature);
      } else if (provider === 'anthropic') {
        if (!apiKey || !apiKey.trim()) {
          return res.status(400).json({
            success: false,
            error: 'Chave da Anthropic (Claude) não informada. Por favor, informe sua chave de API (sk-ant-...) nas configurações de IA.'
          });
        }
        rawOutput = await callAnthropic(apiKey.trim(), model, userPrompt, temperature);
      } else {
      ...
      // If the user explicitly configured OpenAI or Anthropic and it failed (e.g. 401 Invalid Key, 429 Quota), return explicit error!
      ...
      if (validConfiguredWorkflows.length > 0) {
        if (workflowArchitectureMode === 'single_consolidated') {
          ...
          // If cw.curlItems.length > 1, it builds an internally chained multi-REST workflow!
    ...
    if (workflowArchitectureMode === 'single_consolidated' && sanitizedWorkflows.length > 1) {
      ...
      // Collect all REST and middle code nodes across all workflows
